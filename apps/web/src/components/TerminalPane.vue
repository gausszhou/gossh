...
import { getSession, createSession, isCredentialError } from '../utils/api'
import { t } from '../utils/i18n'
import { logger } from '../utils/logger'

const props = defineProps<{
    sessionId: string
    // 会话绑定的主机清单 id:会话消失时凭它复活同 id 会话
    hostId: string
...
// 其他设备销毁、进程退出)用主机记录直接重建同 id 会话(服务端按记录的
// spec 复活,run_count+1)。重建缺少凭据 → 上报上层弹凭据框。
...
async function rebuildSession(): Promise<string | null> {
    logger.info('attach', 'session %s gone, rebuilding on host %s', props.sessionId, props.hostId)
    try {
        const s = await createSession({ host_id: props.hostId, id: props.sessionId })
        logger.info('attach', 'session rebuilt: %s', s.id)
        return s.id
    } catch (err) {
        if (isCredentialError(err)) {
            const message = err instanceof Error ? err.message : String(err)
            emit('credential-required', message)
            return null
        }
...
function attach() {
    ...
    logger.info('attach', 'attach session=%s (host=%s)', props.sessionId, props.hostId)
...
// reattach:凭据弹窗确认并重建会话成功后,由上层调用重新附着
function reattach() {
    attach()
}
