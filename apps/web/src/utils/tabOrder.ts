...
// 参考 gotty 的 tabOrder 实现(见 docs/adr 与 gotty 项目同一特性)。
import { logger } from './logger'

export const TAB_ORDER_KEY = 'gossh.tabOrder'

// loadTabOrder 读取持久化的页签顺序(页签 id 数组)。
// 顺序中可能残留已销毁/已移除的 id:渲染时按现存页签过滤,无害。
...
// saveTabOrder 持久化当前页签显示顺序(ids 为页签 id 数组)。
