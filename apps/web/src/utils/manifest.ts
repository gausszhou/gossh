...
export interface ManifestEntry {
    id: string // 会话 id(16 位 base36)
    hostId: string // 绑定的主机清单 id
    createdAt: number // unix 毫秒
    lastSeen: number // unix 毫秒,最近一次在本设备打开
    title?: string // 程序设置的标题(OSC 0/2);可空,页签回退到主机名
}
    ...
    try {
        ...
        return parsed.filter(
            (e): e is ManifestEntry =>
                e && typeof e.id === 'string' && e.id.length === ID_LENGTH && typeof e.hostId === 'string',
        )
    } catch (err) {
...
// removeFromManifest 从清单移除一个会话;服务端记录保留(deleted 记录),
// 但本设备不再展示它(页签已关闭 = 会话已销毁,清单同步移除)。
