...
var (
	...
	// ErrNoHost is returned when a fresh session (no record to
	// resurrect) is created without a host reference.
	ErrNoHost = errors.New("no host given")
)

// ConnectSpec identifies what a session connects to. It is the gossh
// counterpart of gotty's "command/args": a session is a connection to a
...
// The api layer injects the real factory: resolve the host chain,
// dial it, and wrap the connection in ssh.Tty.
type TerminalFactory func(spec ConnectSpec, opts ...terminal.Option) (Terminal, error)
...
// WithTerminalFactory sets the factory after construction (used by the
// api layer, which builds the dial factory from the server's own
// dependencies).
func (m *Manager) WithTerminalFactory(factory TerminalFactory) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.factory = factory
}
...
// powers the agent-driving API (GET /screen, POST /wait).
...
func NewManager(options ...Option) *Manager {
	...
	if m.factory == nil {
		// No factory injected → no sessions can be created; the api
		// layer always injects one in production.
		m.factory = func(spec ConnectSpec, opts ...terminal.Option) (Terminal, error) {
...
// Create starts a new session connecting to spec, letting the server
// generate the session id (legacy clients that do not send one).
func (m *Manager) Create(spec ConnectSpec, termOpts ...terminal.Option) (*Session, error) {
	sess, _, err := m.CreateWithID("", spec, termOpts...)
	return sess, err
}
...
//     **resurrected**: the recorded connect spec (host) is used to
//     rebuild a session with the same id and run_count is incremented;
//   - otherwise a fresh session is started for the given spec.
//
// An empty id makes the server generate one (legacy clients).
func (m *Manager) CreateWithID(id string, spec ConnectSpec, termOpts ...terminal.Option) (*Session, bool, error) {
	m.mu.Lock()
	...
	// 全新会话必须携带主机引用;复活会话在下方使用记录的 spec
	if !recorded && spec.HostID == "" {
		m.mu.Unlock()
		return nil, false, ErrNoHost
	}
	...
	s := New(id, term, WithScreenMirror(mirror), withAnswerQueries(m.answerQueries), withSpec(spec))
	m.sessions[s.ID()] = s
...
// Destroy stops the session's connection and removes it from the registry.
