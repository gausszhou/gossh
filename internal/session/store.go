...
type legacyFileStorePayload struct {
	Sessions []legacyMetadata `json:"sessions"`
}

// legacyMetadata is the gotty-era record shape (local command/args),
// which the host-based model has no direct equivalent for.
...
func NewFileStore(path string) (*FileStore, error) {
	...
	for _, l := range legacy.Sessions {
		// 旧格式(本地命令)无主机引用:命令保留为显示名,HostID 为空
		// (无法复活,需在前端重新选择主机)。
		meta := Metadata{
			ID:        l.ID,
			Spec:      ConnectSpec{Name: l.Command},
			Title:     l.Title,
			State:     l.State,
