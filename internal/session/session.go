...
type Session struct {
	id        string
	term      Terminal
	spec      ConnectSpec // what this session connects to (host)
	createdAt time.Time
...
// withSpec records the connect spec (host) of the session for the REST API.
func withSpec(spec ConnectSpec) SessionOption {
	return func(s *Session) {
		s.spec = spec
	}
}
...
type StateDescription struct {
	...
	Spec      ConnectSpec `json:"spec"`
	Exited    bool        `json:"exited"`
	Title     string      `json:"title,omitempty"` // 显示名(空 = 自动编号)
	CreatedAt string      `json:"created_at"`
}

// Spec returns the connect spec of the session.
...
func (s *Session) SSHClient() (*ssh.Client, error) {
	...
	if c, ok := s.term.(clienter); ok {
		client := c.SSHClient()
		if client == nil {
			return nil, errors.New("session ssh client is gone")
		}
		return client, nil
	}
	return nil, errors.New("session does not expose an ssh client")
}
