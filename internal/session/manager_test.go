...
func testSpec(name string) ConnectSpec {
	return ConnectSpec{HostID: name, Name: name, Addr: "127.0.0.1:22", User: "root"}
}

func stubFactory() (TerminalFactory, *stubTerminal) {
	stub := newStubTerminal("mock")
	return func(spec ConnectSpec, opts ...terminal.Option) (Terminal, error) {
		// 记录真实入参,供 spec 断言使用
		stub.command = spec.Name
		stub.args = nil
		return stub, nil
	}, stub
}
...
func perCallFactory() TerminalFactory {
	return func(spec ConnectSpec, opts ...terminal.Option) (Terminal, error) {
		return newStubTerminal(spec.Name), nil
	}
}
...
func TestManagerLifecycle(t *testing.T) {
	...
	sess, err := m.Create(testSpec("mock"))
	if err != nil {
...
func TestManagerMaxSession(t *testing.T) {
	...
	if _, err := m.Create(testSpec("mock")); err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
	if _, err := m.Create(testSpec("mock")); !errors.Is(err, ErrTooManySessions) {
...
func TestFileStoreRoundTrip(t *testing.T) {
	...
	_ = store.Record(Metadata{ID: "a1", Spec: testSpec("bash"), State: "idle", CreatedAt: 111})
	_ = store.Record(Metadata{ID: "a2", Spec: testSpec("top"), State: "running", CreatedAt: 222})
	...
	if byID["a1"].Spec.HostID != "bash" || byID["a1"].State != "idle" {
		t.Fatalf("unexpected a1 record: %+v", byID["a1"])
	}
	if byID["a2"].Spec.HostID != "top" || byID["a2"].State != "running" {
...
func TestManagerKeepsHistoryAfterDestroy(t *testing.T) {
	factory, _ := stubFactory()
	m := NewManager(WithTerminalFactory(factory))
	s1, _ := m.Create(testSpec("mock"))
	_, _ = m.Create(testSpec("mock"))
...
func TestManagerHistorySurvivesRestart(t *testing.T) {
	...
	history := m2.History()
	if len(history) != 1 || history[0].ID != s.ID() || history[0].Spec.HostID != "mock" {
...
func TestCreateWithIDIdempotent(t *testing.T) {
	...
	s1, created, err := m.CreateWithID(id, testSpec("mock"))
	if err != nil || !created {
		t.Fatalf("expected fresh create, got created=%v err=%v", created, err)
	}
	s2, created, err := m.CreateWithID(id, testSpec("mock"))
	if err != nil || created {
...
func TestCreateWithIDResurrectsRecordedSession(t *testing.T) {
	...
	s1, _, err := m.CreateWithID(id, testSpec("mock"))
	if err != nil {
	...
	s2, created, err := m.CreateWithID(id, testSpec("totally-different"))
	if err != nil {
	...
	if s2.Command() != "mock" || s2.Spec().HostID != "mock" {
		t.Fatalf("resurrected session must use the recorded spec, got %s %+v", s2.Command(), s2.Spec())
	}
...
func TestManagerStatus(t *testing.T) {
	...
	factory := func(spec ConnectSpec, opts ...terminal.Option) (Terminal, error) {
		lastStub = newStubTerminal(spec.Name)
		return lastStub, nil
	}
	m := NewManager(WithTerminalFactory(factory))

	s1, _ := m.Create(testSpec("mock"))
	s2, _ := m.Create(testSpec("mock"))
...
func TestFileStoreMigratesLegacyArray(t *testing.T) {
	...
	if meta, ok := reloaded.Get("a1"); !ok || meta.Spec.Name != "bash" || meta.State != "idle" {
