...
// ApplyEnv fills flags that were not explicitly set from GOSSH_* env vars,
// e.g. GOSSH_PORT for the flag --port.
func ApplyEnv(cmd *cobra.Command, mappings map[string]string, options ...interface{}) error {
	for flagName, fieldName := range mappings {
		...
		envName := "GOSSH_" + strings.ToUpper(strings.ReplaceAll(flagName, "-", "_"))
